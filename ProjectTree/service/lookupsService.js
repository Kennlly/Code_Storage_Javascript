...
const fetchLookupPattern = async (apiEndpoint) => {
   ...
   while (true) {
      ...
      try {
         ...
         if (data === false) {
            generalLogger.error(`${funcName} ${funcArgus} - Getting data from Genesys ERROR!`);
            return false;
         }

         const partial = data["entities"];
         if (JSON.stringify(data) === "{}" || !partial || partial.length === 0) {
            generalLogger.error(`${funcName} ${funcArgus} - Unexpected EMPTY payload!`);
            return false;
         }
...
export const fetchGroup = async () => {
   try {
      ...
      generalLogger.error(`[fetchGroup Func] Catching ERROR - ${err}.`);
      return false;
   }
};
